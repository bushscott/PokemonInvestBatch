...
| burn window / at risk | a selling card's burn window = days its sales rate takes to fill a ~30-row bucket, after which rows roll off forever; a card is "at risk" when that window is shorter than the revisit cycle — it survives only by fast-tracking. The scheduler guarantees a visit partway into the window (prevention outranks even cap-hit revisits) — **at 0.4 of it for cards selling ≥1/day, 0.5 for the rest**, since only a card that sells can get hotter between visits | `VisitPriority.cs` burn-window tier, `crawl.cards_at_risk`, "Cards at risk of missing sales (predictive)" widget |
...
| safety fraction | how much of its burn window a card may spend before a revisit is due. It buys exactly one thing: revisiting at fraction *f* absorbs a card getting up to **1/f** times hotter between visits — 0.5 absorbs a doubling, 0.4 absorbs 2.5×. Mega Gardevior EX #32 accelerated 2.05× on 2026-08-10, a hair past 0.5, and lost ~25 PSA 10 rows. The knob to turn down if that happens again | `ScraperOptions.HotBurnWindowSafetyFraction` (config, no rebuild), `VisitPriority.SafetyFractionFor` |
