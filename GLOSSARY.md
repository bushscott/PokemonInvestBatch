...
| shape / fingerprint | structure-only hash of a page (layout, not values); a never-seen hash is archived, HTML and all | `PageFingerprint.cs`, `shapes` table, `/var/lib/pokemon/shapes/` |
| vocabulary | the names a shape uses, qualified by bucket (`chart_data:psa10`); a name seen in no earlier shape is the site changing its markup and the only thing that alerts — a novel hash alone is usually just a card with less data | `PageShapeVocabulary.cs`, alert "New page element observed" |
