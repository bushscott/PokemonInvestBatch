...
| sale-cap / bucket at cap | the site keeps only the last 30–50 sales per grade; a card is "at cap" when a visit proves sales scrolled off unseen since last time — it then gets fast-tracked revisits | `SalesObservation.cs`, `cards.any_bucket_at_cap`, `crawl.cards_at_cap`, "Cards selling faster than we can track" widget |
